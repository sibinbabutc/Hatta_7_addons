...
class account_fiscalyear_close(osv.osv_memory):
    ...
    def data_save(self, cr, uid, ids, context=None):
        ...
        obj_ana_acc = self.pool.get('account.analytic.account')
        ...
        query_2nd_part = ""
        query_2nd_part_args = []
        ana_account_ids = obj_ana_acc.search(cr, uid, [], context=context)
        for ana_account in ana_account_ids:
            ctx = context.copy()
            ctx['fiscalyear'] = fy_id
            ctx['ann_account_ids'] = [ana_account]
            for account in obj_acc_account.browse(cr, uid, account_ids, context=ctx):
