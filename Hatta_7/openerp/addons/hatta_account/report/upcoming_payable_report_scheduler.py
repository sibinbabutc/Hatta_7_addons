...
class upcoming_payable_report_scheduler(JasperDataParser.JasperDataParser):
    def __init__(self, cr, uid, ids, data, context):
        super(upcoming_payable_report_scheduler, self).__init__(cr, uid, ids, data, context)
...
#         date_to = data['form']['to_date']
        ...
        due_date_from = datetime.today()
        from_date = due_date_from.strftime("%d/%m/%Y") or ''
        due_date_to = due_date_from + timedelta(days=10)
        to_date = due_date_to.strftime("%d/%m/%Y") or ''
        report_heading = "Upcoming Payable Report of %s till %s"%(from_date,to_date)
        vals = {
    ...
    def generate_records(self, cr, uid, ids, data, context=None):
        ...
        move_line_pool = self.pool.get('account.move.line')
        condition = []
        condition.append(('account_id.type','=','payable'))
        condition.append(('reconcile_id','=',False))
        condition.append(('credit', '>', 0.00))
...
jasper_report.report_jasper('report.upcoming_payable_report_scheduler.jasper', 'account.move.line', parser=upcoming_payable_report_scheduler)
