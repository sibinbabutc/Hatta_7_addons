...
class voucher_report(JasperDataParser.JasperDataParser):
    def __init__(self, cr, uid, ids, data, context):
        super(voucher_report, self).__init__(cr, uid, ids, data, context)
    ...
    def get_bank_name(self, cr, uid, account_id, context=None):
        result = ''
        account_pool = self.pool.get('account.account')
        journal_pool = self.pool.get('account.journal')
        bank_pool = self.pool.get('res.partner.bank')
        journal_ids = journal_pool.search(cr, uid, ['|', ('default_debit_account_id', '=', account_id),
                                                    ('default_credit_account_id', '=', account_id),
                                                    ('type', '=', 'bank')])
        bank_ids = bank_pool.search(cr, uid, [('journal_id', 'in', journal_ids)],
                                    context=context)
        print bank_ids
        if bank_ids:
            bank_obj = bank_pool.browse(cr, uid, bank_ids[0], context=context)
            result = bank_obj.bank_name or ''
        return result
    ...
    def generate_records(self, cr, uid, ids, data, context):
        ...
        for move_obj in move_pool.browse(cr, uid, ids, context=context):
            ...
            for line in move_obj.line_id:
                ...
                tot_amount += abs(line.debit or 0.00)
...
jasper_report.report_jasper('report.voucher.report.jasper', 'account.move', parser=voucher_report)
