...
class subledger_report(JasperDataParser.JasperDataParser, CommonPartnersReportHeaderWebkit):
...
#             period_ids = [-1]
    ...
    def get_rec_balance(self, cr, uid, rec_id, rec_partial_id, move_line_obj,
                        date_from, date_to, field='debit', context=None):
        ...
        if rec_id:
            rec_obj = rec_pool.browse(cr, uid, rec_id, context=context)
            for rec_line in rec_obj.line_id:
                reco_date = datetime.strptime(rec_line.date, "%Y-%m-%d").strftime("%Y-%m-%d")
                if reco_date >= date_from and reco_date <= date_to and rec_line.id != move_line_obj.id:
                    if field == 'debit':
                        debit_sum = rec_line.debit
                    if field == 'credit':
                        credit_sum = rec_line.credit
            if field == 'debit':
                used_amount = self.rec_used_amt.get(rec_obj.id, {}).get('debit', 0.00)
                debit_sum -= used_amount
            if field == 'credit':
                used_amount = self.rec_used_amt.get(rec_obj.id, {}).get('credit', 0.00)
                credit_sum -= used_amount
        if rec_partial_id:
            rec_obj = rec_pool.browse(cr, uid, rec_partial_id, context=context)
            for rec_line in rec_obj.line_partial_ids:
                reco_date = datetime.strptime(rec_line.date, "%Y-%m-%d").strftime("%Y-%m-%d")
                if reco_date >= date_from and reco_date <= date_to and rec_line.id != move_line_obj.id:
                    if field == 'debit':
                        debit_sum = rec_line.debit
    ...
    def generate_records(self, cr, uid, ids, data, context):
        ...
        if job_ledger and job_id:
            filter_type = ('payable', 'receivable', 'other', 'liquidity', 'consolidation')
        if account_data:
        ...
        exclude_recosile = False
        show_match = form_data.get('include_matching_tran', False)
        if show_match:
            exclude_recosile = False
        if partner_data:
        ...
        for account in account_ids:
            partner_move_line_data = self.get_partners_move_lines_ids(account, 'filter_date', date_start,
                                                                      date_end, 'all', exclude_recosile,
                                                                      partner_ids, job_id, cost_center)
            if not partner_move_line_data:
                continue
            for partner_id in partner_move_line_data:
                ...
                if not ini_bal_done.get(partner_id, False):
                    ...
                    initial_balance = self._compute_partners_initial_balances(account_ids,
                                                                              ...
                                                                              exclude_reconcile=exclude_recosile)
                    open_debit = 0.00
                    open_credit = 0.00
                    ...
                    if open_debit != 0.00 or open_credit != 0.00:
                        balance = open_debit - open_credit
                        credit = debit = 0.00
                        if balance < 0.00:
                            credit = balance
                        elif balance > 0.00:
                        ...
                        if not partner_balance.has_key(partner_id):
                            partner_balance[partner_id] = open_debit - open_credit
                if not partner_balance.has_key(partner_id):
                ...
                partner_address = ''
                if partner_id and not job_ledger:
                    partner_obj = partner_pool.browse(cr, uid, partner_id, context=context)
                    partner_address = partner_pool._display_address(cr, uid, partner_obj,
                                                                    context=context)
                ...
                for line in lines:
                    ...
                    if not show_match:
                        recon_id = line.get('rec_id', False)
                        recon_par_id = line.get('prec_id', False)
                        if recon_id or recon_par_id:
                            if debit != 0.00:
                                field = 'credit'
                            if credit != 0.00:
                                field = 'debit'
                            rec_credit, rec_debit = self.get_rec_balance(cr, uid, recon_id, recon_par_id,
                                                                         move_line_obj, date_start, date_end,
                                                                         field, context=context)
#                             credit = debit = 0.00
                            rec = recon_id or recon_par_id
                            if debit != 0.00:
                                real_debit = debit
                                if debit <= rec_credit:
                                    debit = 0.00
                                    if not self.rec_used_amt.has_key(rec):
                                        self.rec_used_amt[rec] = {
                                                                  'debit': 0.00,
                                                                  'credit': 0.00
                                                                  }
                                    self.rec_used_amt[rec]['credit'] = real_debit
                                elif debit > rec_credit:
                                    debit = debit - rec_credit
                                    if not self.rec_used_amt.has_key(rec):
                                        self.rec_used_amt[rec] = {
                                                                  'debit': 0.00,
                                                                  'credit': 0.00
                                                                  }
                                    self.rec_used_amt[rec]['credit'] = rec_credit
                            if credit != 0.00:
                                real_credit = credit
                                if credit <= rec_debit:
                                    credit = 0.00
                                    ...
                                    self.rec_used_amt[rec]['debit'] = real_credit
                                elif credit > rec_debit:
                                    credit = credit - rec_debit
                                    if not self.rec_used_amt.has_key(rec):
                                        self.rec_used_amt[rec] = {
                                                                  'debit': 0.00,
                                                                  'credit': 0.00
                                                                  }
                                    self.rec_used_amt[rec]['debit'] = rec_debit
                    if debit != 0.00 or credit != 0.00:
                        ...
                        if date_end and date_maturity:
                            ...
                            if float(due_days) <= 30.00:
                                ...
                                vals['other'] += balance
                        result.append(vals)
#                     if line.get('rec_id', False) and not show_match:
#                         continue
#                     debit = line.get('debit', 0.00)
#                     credit = line.get('credit', 0.00)
#                     balance = debit - credit
#                     sign = 1.00
#                     account_type = move_line_obj.account_id.type
#                     line['account_type'] = account_type
#                     if account_type == 'payable':
#                         sign = -1.00
#                     balance = balance * sign
#                     recon_id = line.get('rec_id', False)
#                     recon_par_id = line.get('prec_id', False) or recon_id or False
#                     if balance == 0.00:
#                         continue
#                     if show_match:
#                         if not move_line_obj.move_id.skip_check:
#                             real_line.append(line)
#                     elif not show_match and recon_par_id:
# #                         if balance < 0.00:
#                         reconsile_line.append(line)
#                         if not reco_data.get(recon_par_id, False):
#                             reco_data[recon_par_id] = 0.00
#                         reco_data[recon_par_id] += balance * sign
# #                         else:
# #                             real_line.append(line)
#                     elif not show_match and not recon_par_id:
#                         real_line.append(line)
#                 print reco_data
...
#                     credit = line.get('credit', 0.00)
#                     balance = debit - credit
#                     rec_id = line.get('rec_id', False)
#                     recon_par_id = line.get('prec_id', False) or rec_id or False
#                     rec_value = reco_data.get(recon_par_id, 0.00)
#                     if rec_value == 0.00 and abs(balance) == 0.00:
#                         continue
...
#                     elif rec_value > abs(balance):
#                         balance = 0.00
#                         reco_data[recon_par_id] = rec_value + balance
...
#                     recon_par_id = line.get('prec_id', False) or rec_id or False
#                     rec_value = reco_data.get(recon_par_id, 0.00)
#                     print rec_value,"<---------------REC VALUE------->",abs(balance)
#                     if rec_value == 0.00 and abs(balance) == 0.00:
#                         continue
#                     if rec_value != 0.00:
#                         rec_value = rec_value * sign
#                         
#                         
#                         credit = debit = 0.00
#                         if rec_value < 0.00:
...
#                                 }
#                         result.append(vals)
        if not result and partner_data:
            partner_obj = partner_pool.browse(cr, uid, partner_data[0], context=context)
            partner_name = partner_obj.name or ''
            partner_code = partner_obj.partner_ac_code or ''
            if partner_code:
                partner_name = "%s\n%s"%(partner_code, partner_name)
            ...
            for partner in partner_address_list:
                if partner:
                    new_partner_address_list.append(partner)
            partner_address = "\n".join(new_partner_address_list)
            result.append({'customer_name': partner_name,
                           'partner_address': partner_address and partner_address.upper() or '',})
        return result
