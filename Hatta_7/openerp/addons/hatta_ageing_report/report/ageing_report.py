...
class aging_report_summary(JasperDataParser.JasperDataParser, aged_trial_report):
    def __init__(self, cr, uid, ids, data, context):
        self.total_account = []
        data['report_type'] = data['form']['report_type']
        super(aging_report_summary, self).__init__(cr, uid, ids, data, context)
    ...
    def generate_parameters(self, cr, uid, ids, data, context):
        ...
        if print_type == 'customer':
            header = "DEBTORS AGEING SUMMARY AS ON %s"%(str(date))
        elif print_type == 'supplier':
            header = "CREDITOR AGEING SUMMARY AS ON %s"%(str(date))
        else:
            header = "AGEING SUMMARY AS ON %s"%(str(date))
        vals = {
                'header': header,
    ...
    def _get_lines(self, form):
        ...
        self.cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit), l.id \
                    ...
                    AND account_account.active\
                    AND (l.date <= %s)\
                    GROUP BY l.partner_id,l.id ', (tuple(move_state), tuple(self.ACCOUNT_TYPE), tuple(partner_ids), self.date_from,))
        t = self.cr.fetchall()
        for i in t:
            totals[i[0]] = i[1]
        ...
        if self.direction_selection == 'future':
            self.cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit), l.id \
                        ...
                        AND account_account.active\
                    AND (l.date <= %s)\
                        GROUP BY l.partner_id,l.id', (tuple(move_state), tuple(self.ACCOUNT_TYPE), self.date_from, tuple(partner_ids),self.date_from,))
            t = self.cr.fetchall()
            for i in t:
                future_past[i[0]] = i[1]
        ...
        for i in range(5):
            ...
            self.cr.execute('''SELECT rp.name as partner_name, rp.partner_ac_code as partner_code,
                    SUM(l.debit-l.credit) as balance, l.doc_no as ref,
                    (case when ai.name is not null and ai.type = 'out_refund' then ai.name
                    else l.name end) as remark,
                    ...
                    left join account_account account_account on account_account.id = l.account_id
                    ...
                    WHERE
                        ...
                        AND ''' + dates_query + '''
                    AND (l.date <= %s)
                    GROUP BY rp.name, rp.partner_ac_code, l.doc_no, l.name,
                    l.date_maturity, l.analytic_account_id, rp.id, l.date, account_account.type,
                    l.reconcile_partial_id, l.reconcile_id, ai.name, ai.type
                    ''', args_list)
        ...
        for j in real_line:
            ...
            if balance != 0.00:
                ...
                vals = {
                        ...
                        'ref': j.get('ref', ''),
                        'remark': j.get('remark', ''),
                        'date_due': j.get('due_date', False) and datetime.strptime(j['due_date'], '%Y-%m-%d') or '',
                        'cost_center': cc_code,
                        'doc_date': j.get('doc_date', False) and datetime.strptime(j['doc_date'], '%Y-%m-%d') or '',
                        'pt_0': 0.00,
                        'pt_1': 0.00,
                        'pt_2': 0.00,
                        'pt_3': 0.00,
...
#                     balance = j.get('balance', 0.00)
                sign = 1
                if balance < 0.00:
                    vals['unmatched'] += balance * -1
                else:
                    vals["pt_%s"%(str(i))] = (balance)
                vals['bal'] = balance
                res.append(vals)
    ...
    def generate_records(self, cr, uid, ids, data, context):
        ...
        for item in result:
            if not item['ref']:
                ...
                final_result.append(item)
        result = sorted(final_result, key=operator.itemgetter('partner_name', 'doc_date'))
        return result
    
jasper_report.report_jasper('report.partner.ageing.jasper', 'partner.ageing.report.wizard', parser=aging_report_summary)
