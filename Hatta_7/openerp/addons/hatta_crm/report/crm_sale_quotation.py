...
class crm_sale_quote(JasperDataParser.JasperDataParser):
    ...
    def generate_parameters(self, cr, uid, ids, data, context):
        ...
        address = "Phone : " + comp_obj.phone + ",Fax : " + comp_obj.fax + " - P.O. Box : " + \
                            comp_obj.zip + " - " + comp_obj.city + ', ' + (comp_obj.country_id and comp_obj.country_id.name)
        ...
        for crm_obj in crm_pool.browse(cr, uid, ids, context=context):
            ...
            for line in crm_obj.product_lines:
                if not line.selected_pol_id and line.pol_ids and line.overide_sale_price == 0.00:
                    ...
                    try:
                        sequence = float(line.sequence_no or 999)
                    except:
                        sequence = line.sequence_no
                    gen_lines.append({
                                     'float_seq': sequence or '',
            ...
            temp_list = sorted(temp_list, key=operator.itemgetter('float_seq'))
            gen_lines = sorted(gen_lines, key=operator.itemgetter('float_seq'))
            final_list = []
            if option_lines:
...
jasper_report.report_jasper('report.crm.sale.quote', 'crm.lead', parser=crm_sale_quote)
